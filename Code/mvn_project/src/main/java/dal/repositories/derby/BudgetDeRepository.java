...
/**
 * BudgetDeRepository give the access methodes for handle the budget into derby
 * persistence
 *
...
public class BudgetDeRepository implements IBudgetRepository {
	...
	public IDALBudgetEntity getBudget(int id) throws DALException {
		...
		try {
			budget = (BudgetDeEntity) session
					.createCriteria(BudgetDeEntity.class)
	...
	public List<IDALBudgetEntity> getBudgetsByClient(int id)
		...
		try {
			budgetEntities = session.createQuery(
					"from BudgetDeEntity where clientId = :clientid")
