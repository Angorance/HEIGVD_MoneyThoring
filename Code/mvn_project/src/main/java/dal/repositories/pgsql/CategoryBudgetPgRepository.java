...
public class CategoryBudgetPgRepository implements ICategoriesBudgetRepository {
     ...
     * {@inheritDoc}
     */
    public CategoryBudgetPgRepository(Session session, Transaction transaction) {
     ...
     */
    @Override
    public IDALCategoriesbudgetEntity getCategoriesBudget(int budget_id, int category_id) throws DALException {
        IDALCategoriesbudgetEntity categoriesbudgetEntity = null;

        try {
            categoriesbudgetEntity = (CategoriesbudgetPgEntity) session.createCriteria(CategoriesbudgetPgEntity.class)
                    .add(Restrictions.and(Restrictions.eq("category_id", category_id),
    ...
    public List<IDALCategoriesbudgetEntity> getCategoriesBudgets() throws DALException {
        ...
        try {
            categoriesbudgetEntities = session.createQuery("from CategoriesbudgetPgEntity ").list();
    ...
    public void update(IDALCategoriesbudgetEntity categoriesBudget) throws DALException {
        ...
        if (categoriesBudget.getClass() == CategoriesbudgetPgEntity.class)
            categoriesbudgetPgEntity = (CategoriesbudgetPgEntity) categoriesBudget;
        else
    ...
    public void delete(IDALCategoriesbudgetEntity categoriesBudget) throws DALException {
        ...
        try {
            categoriesbudget = (CategoriesbudgetPgEntity) session.createCriteria(CategoriesbudgetPgEntity.class)
                    .add(Restrictions.and(Restrictions.eq("category_id", categoriesBudget.getCategoryId()),
                            Restrictions.eq("budget_id",categoriesBudget.getBudgetId())))
                    .uniqueResult();
