...
import static gui.Utility.toRGBCode;
...
public class Controller_listCategory implements Initializable, IController {
	...
	private class CategoryDisplayer extends HBox {
		...
		CategoryDisplayer(CategoryLogic c) {
			...
			lblCategorie
					.setStyle("-fx-background-color: white;-fx-font-size: 15");
		...
		public void redraw() {
			...
			this.setStyle("-fx-background-color: " + toRGBCode(
					Color.valueOf(cat.getColor()))
	 ...
	 *
	 * @param cat category to eventually modify. if null we open the form in
	 * 		order to create a category
	 */
	public void callform(CategoryLogic cat) {
		...
		FXMLLoader l = new FXMLLoader(
				getClass().getResource("/gui/view/formCategory.fxml"));
	...
	/**
	 * Called by the form controller. Close the form, and retrieve the created
	 * object if existing
	 *
